...
### DESeq2
if (is.element('DESeq2', installed.packages()[,1]))
{
  suppressPackageStartupMessages(require('DESeq2'));
} else
{
  install.packages('DESeq2', repos=MIRROR);
...
{
  install.packages('edgeR', repos=MIRROR);
...
option_list <- list(
  ...
  make_option(c("-D", "--sartools-rdata"), type="character", dest="rdata", help="The SARTools R session RData output file path [mandatory]", metavar="sartools_rdata"),
  make_option(c("-t", "--deseq2-type-transform"), type="character", dest="typeTransform", default="VST", help="The DESeq2 type transformation [VST, rlog]", metavar="type_transform"),
...
if (file.access(opt$rdata, mode=4) == -1) {
  error(logger, paste("SARTools R session RData output file ", opt$rdata, " does not exist or is not readable.", sep=""))
  stop(sprintf("SARTools R session RData output ( %s ) does not exist or is not readable.", opt$rdata))
} else
{
  debug(logger, paste("OK SARTools R session RData output file ", opt$rdata, " does exist and is readable", sep=""))
  rdata <- file_path_as_absolute(opt$rdata)
}

if (is.null(opt$typeTransform)) {
  error(logger, paste("DESeq2 type transformation string cannot be null. It is mandatory to set a non null type transfomation string.", sep=""))
  stop(sprintf("DESeq2 type transformation string ( %s ) is null. It is mandatory to set a non null main effect string [VST or rlog].", opt$typeTransform))
} else 
{
  typeTransform <- opt$typeTransform
  debug(logger, paste("DESeq2 type transformation string is now set to ", typeTransform, sep=""))
}
...
## order genes: ordering by log2FoldChange then by adjusted pvalue (default)
...
## load SARTools RData
info(logger, paste0("Loading SARTools session RData ouput file: ", rdata))
load(rdata)

## get the differential expression object from SARTools RData
info(logger, paste0("Getting differential expression object from SARTools session RData ouput file"))
if (exists("out.DESeq2")) {
  deo <- out.DESeq2$dds
} else if (exists("out.edgeR")) {
  deo <- out.edgeR$dge
} else {
  stop("There is no DESeqDataSet nor a DGEList loaded from SARTools RData.")
}
info(logger, paste0("Differential expression object is ", class(deo)))

## get the transformed counts
## DESeq2
if (class(deo)=="DESeqDataSet") {
  info(logger, paste0("Getting the transformed counts from DESeq2 dataset"))
  if (typeTransform == "VST") {
    counts.trans <- assay(varianceStabilizingTransformation(deo))
  } else {
    counts.trans <- assay(rlogTransformation(deo))
  }
## edgeR
} else if (class(deo)=="DGEList") {
  info(logger, paste0("Getting the transformed counts from edgeR DGE list"))
  counts.trans <- cpm(deo, prior.count=2, log=TRUE)
} else{
...
if (nrow(df) < genesCount || genesCount == -1) {
  genesCount <- nrow(df) 
}
counts.trans <- subset(counts.trans, rownames(counts.trans)%in%df$Id[1:genesCount])

# and reorder transformed counts using sorted deg
counts.trans <- counts.trans[order(match(rownames(counts.trans), df[1:genesCount,"Id"])),]

## Centering
info(logger, paste("Centering matrix counts by row", sep=""))
counts.trans <- counts.trans - rowMeans(counts.trans)
...
info(logger, paste("before: ", paste(colnames(counts.trans), collapse = ","), sep=""))
colnames(counts.trans) <- colnames
info(logger, paste("after: ", paste(colnames(counts.trans), collapse = ","), sep=""))

## clustering
if (class(deo)=="DESeqDataSet") {
  fout <- paste0(basename(file_path_sans_ext(deg)), "_heatmap_n", genesCount, "_sort", sortDEG, "_", typeTransform, ".pdf")
} else if (class(deo)=="DGEList") {
  fout <- paste0(basename(file_path_sans_ext(deg)), "_heatmap_n", genesCount, "_sort", sortDEG, "_log2cpm", ".pdf")
}
info(logger, paste("Setting plot output file name: ", fout, sep=""))
info(logger, paste("Running the clustering", sep=""))
# change default colors palette
pheatmap(counts.trans, cellheight = 10, filename = fout, annotation_col = annotation_col, cluster_rows = FALSE,
         color = colorRampPalette(rev(brewer.pal(n = 11, name = "Spectral")))(200))

unlink('Rplots.pdf')

# R session
debug(logger, sessionInfo())
